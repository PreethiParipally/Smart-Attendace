...
<div id="container">
    ...
    <p align="center" id="bd">
        <span align="center" style="font-size: 28px;"><b>Search the Attendance of Students</b></span>
        <br><br><button class="cls" onclick="display(1)">Search By Date</button><br><br>
        <button class="cls" onclick="display(2)">View Attendance</button>
        <div id="con">
            <form method="GET" action="" style="display: none;" id="search_date">
                <div class="row">
                    <div class="col-sm-3"></div>
                    <div class="col-sm-3"><label>Subject: </label><input type="text" class="form-control" name="subj" placeholder="e.g: C,Java,Chemistry,Applied Physics,M1..." maxlength=8 required></div>
        ...
        <?php
            ...
            if (isset($_GET['Login'])) {
                ...
                if ($type == 'date'){
                    ...
                    $subj = sanitize($_GET['subj']);
                    $date = sanitize($_GET['date']);
                    $uname = sanitize($_SESSION['name']);
                    $sql1= "SELECT s.roll as roll,s.name as name,s.class as class FROM student s inner join user u on u.id=s.uid and u.username = '$uname' ";
                    ...
                    if (mysqli_num_rows($rslt)>0) {
                        ...
                        if (mysqli_num_rows($rlt)>0) {
                            ...
                            // echo "Name: <b>$name</b> || Class: <b>$class</b> || Roll: <b>$new_roll</b><br>Attendance status on Date: $date and for subject: $subj : <b>$status</b>";
                        ...
                        echo "</div>";
                            ...
                            echo "Wrong Subject or Date entered!";
                        }
                    }
                    else{
                        echo "Wrong Subject or Date entered!";
